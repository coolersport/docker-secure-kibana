...
RUN yum update -y && \
    ...
    yum install -y dpkg && \
    /usr/share/kibana/bin/kibana-plugin install https://oss.sonatype.org/content/repositories/releases/com/floragunn/search-guard-kibana-plugin/6.2.1-10/search-guard-kibana-plugin-6.2.1-10.zip && \
